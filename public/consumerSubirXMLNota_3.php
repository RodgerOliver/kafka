...
$topic = $consumer->newTopic("subirXMLNota", $topicConf);
$start_offset = RD_KAFKA_OFFSET_STORED < 0 ? RD_KAFKA_OFFSET_END : RD_KAFKA_OFFSET_STORED;
$topic->consumeQueueStart(2, $start_offset, $queue);
...
while (true) {
    ...
    if (null === $msg || $msg->err === RD_KAFKA_RESP_ERR__PARTITION_EOF) {
        ...
        try {
            ...
            logMsg('INFO', var_export($soapReturnXML, true));
...
function logError($data, $error) {
    ...
    $data['dados']['Axml_envio'] = str_replace(array("\r", "\n"), '', $data['dados']['Axml_envio']);
    ...
    file_put_contents($pasta_erro. "/subirXMLNota.log", $line, FILE_APPEND);
}
